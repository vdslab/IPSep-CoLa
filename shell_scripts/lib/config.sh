...
# run_experiment.sh から source されることを想定しますが、
# 単体で読み込まれた場合にも REPO_ROOT を推定できるようにします。
if [ -z "${REPO_ROOT:-}" ]; then
...
# 並列実行のジョブ数設定
# 環境: メモリ12GB実用、CPUコア12
PARALLEL_JOBS=12        # グラフ描画の並列ジョブ数（CPUコア数に合わせて調整）
PARALLEL_PLOTS=8        # プロット処理の並列数
