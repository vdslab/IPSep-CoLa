...
def main():
    ...
    parser.add_argument(
        "--output", required=True, help="Path to output image file"
    )
    parser.add_argument(
        "--bins", type=int, default=50, help="Number of bins for histogram (default: 50)"
    )
    parser.add_argument(
        "--log-scale", action="store_true", help="Use logarithmic scale for y-axis"
    )
    parser.add_argument(
        "--alpha", type=float, default=0.6, help="Transparency of histograms (default: 0.6)"
    )
